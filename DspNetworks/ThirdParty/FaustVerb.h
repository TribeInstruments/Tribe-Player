...
#include "src/FaustVerb.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct FaustVerbMetaData {
		SN_NODE_ID("FaustVerb");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using FaustVerb = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _FaustVerb, FaustVerbMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
